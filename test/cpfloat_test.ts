  ...
  for (format = minformat; format <= maxformat; format++) {
    ...
    for (subnormal = minsubnormal; subnormal <= maxsubnormal; subnormal++) {
      ...
      for (explim = minexplim; explim <= maxexplim; explim++) {
        fpopts->explim = explim;
        if (mode == 5)
          check_array_stoc_double(tmpin, tmpout, x, prounddown, n, fpopts);
        else if (mode == 6)
          check_array_equi_double(tmpin, tmpout, x, prounddown, n, fpopts);
      }
    }
  }
}
  ...
  for (format = minformat; format <= maxformat; format++) {
    ...
    for (subnormal = minsubnormal; subnormal <= maxsubnormal; subnormal++) {
      ...
      for (explim = minexplim; explim <= maxexplim; explim++) {
        fpopts->explim = explim;
        if (mode == 5)
          check_array_stoc_float(tmpin, tmpout, x, prounddown, n, fpopts);
        else if (mode == 6)
          check_array_equi_float(tmpin, tmpout, x, prounddown, n, fpopts);
      }
    }
  }
}
...
for (i = 0; i < nformats; i++) {
  ...
  select_tests_det_double(yd, (double *)xd, (double *)xd, n, fpopts,
...
for (i = 0; i < nformats; i++) {
  ...
  double xd [] = {nextafter(0, INFINITY),
                  nextafter(snxmin, 0),
                  snxmin,
  ...
  select_tests_det_double(yd, xd, refzerod, n, fpopts, 4, 4, i, i, 0, 1);
  select_tests_det_double(yd, xd, refxmind, n, fpopts, 7, 7, i, i, 0, 1);
  float xf [] = {nextafterf(0, INFINITY),
                 nextafterf(snxmin, 0),
                 snxmin,
...
for (i = 0; i < nformats; i++) {
  ...
  double xd [] = {nextafter(xmax, INFINITY),
                  nextafter(xbound, 0),
                  xbound,
  ...
  select_tests_det_double(yd, xd, refxmaxd, n, fpopts, 7, 7, i, i, -1, 1);
  float xf [] = {nextafterf(xmax, INFINITY),
                 nextafterf(xbound, 0),
                 xbound,
...
tcase_set_timeout(tc1_1, 60);
tcase_set_timeout(tc1_2, 120);
tcase_set_timeout(tc1_3, 180);
